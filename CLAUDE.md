...
3. **External Dependencies**
   ...
   - **PO-token provider (optional, bgutil)**: `POT_PROVIDER_URL` + the `bgutil-ytdlp-pot-provider` plugin (`requirements-potoken.txt`) → `_build_ydl_opts` sets `extractor_args={"youtubepot-bgutilhttp": {"base_url": [url]}}`. Robust fix for SABR-gated videos; a commented sidecar is in `docker-compose.yml`. Wired but not verified end-to-end (needs the provider running).
